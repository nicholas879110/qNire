...
$(function () {
    $( ".navbar ul.nav a").bind("click",function() {
        var aDom = $(this);
...
//    $(".navbar ul.nav a:first").click();

    $("#navbarCollapse-dropdown").bind("click",function(){
        //$(this).children().removeClass("icon-double-angle-down").addClass();
        // $(this).children();

        $("body").toggleClass("navbar-dropdown");
    });
...
//    $("#main-container").load($(dom).attr("view"));

    $("#page-content").load(getContentPath()+$(dom).attr("view"));
...
function switchPage(action,args,callback){
    $("#main-container").load(getContentPath()+action,args,callback);
}
